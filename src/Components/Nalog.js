...
const Modal = ({active, setActive}) => {
    ...
    const Results = () => (
        <div id={"calculate"}>
            <span className={"Money_text"}>Итого можете внести в качестве досрочных:</span>
            <div className={"raschet"}>
                <input className={"custom"} type={"checkbox"} id={"freak"} name={"freak"}/>
                <label htmlFor="freak">{result} рублей&nbsp; <span className={"year"}>в 1-ый год</span></label>
            </div>
            <div className={"raschet"}>
                <input className={"custom1"} type={"checkbox"} id={"freak1"} name={"freak1"}/>
                <label htmlFor="freak1">{result} рублей&nbsp; <span className={"year"}>в 2-ой год</span></label>
            </div>
            <div className={"raschet"}>
                <input className={"custom2"} type={"checkbox"} id={"freak2"} name={"freak2"}/>
                <label htmlFor="freak2">{result} рублей&nbsp; <span className={"year"}>в 3-ий год</span></label>
            </div>
            <div className={"raschet"}>
                <input className={"custom3"} type={"checkbox"} id={"freak3"} name={"freak3"}/>
                <label htmlFor="freak3">{result} рублей&nbsp;  <span className={"year"}>в 4-ый год</span></label>
            </div>
        </div>
    )
    ...
    function handleClick() {
        const max = Number(260000)
        for (let i = value1; i<=max; i++)
        i = setResult(((Number(value1) * Number(12)) * Number(13) / Number(100)));
        setOpen(true)
    }
    ...
    return (
        <div className={active ? "modal active" : "modal"} onClick={() => setActive(false)}>
            <div className="modal_content" onClick={e => e.stopPropagation()}>
                <div className="close" onClick={() => setActive(false)}><svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <p className="text">Используйте налоговый вычет чтобы погасить ипотеку досрочно.  
                    Размер налогового вычета составляет не более 13% от своего официального годового дохода.</p>
                <span className="input_name">Ваша зарплата в месяц</span>
                <input id={"nalog_input"}
                       value={value1} onChange={handleChange}
                       type="number" step="0.01"
                placeholder={"Введите данные"}
                       required/>
                       <div>
                <button type="button" onClick={handleClick} className={"text_calculate"}>Рассчитать</button>
                           { open ? <Results /> : null}</div>
                <div className={"PayOrTime"}>
                <span className={"low"}>Что уменьшаем?</span>
                    <div className={"Lit"}>
                    <button className= {'Time'}>Платеж</button>
                    <button className= {'Time'}>Срок</button>
                    </div>
                </div>
